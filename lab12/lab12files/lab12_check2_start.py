...
class MyApp(object):
    ...
    def draw_lines(self, centerx, centery, length):
        if length >= 3:
            self.canvas.create_line(centerx, centery+length, self.draw_lines(centerx,centery+length,length/2),\
                                    centerx, centery-length, self.draw_lines(centerx,centery-length,length/2), fill="red") 
            self.canvas.create_line(centerx+length, centery, self.draw_lines(centerx+length, centery, length/2),\
                                    centerx-length, centery, self.draw_lines(centerx-length, centery, length/2), fill="green")
